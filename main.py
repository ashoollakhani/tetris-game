...
def draw_text_top_left(text, size, color, surface):
    font = pygame.font.SysFont('comicsans', size, bold=True)
    label = font.render(text, 1, color)
...
def draw_window(surface, grid, score=0):
    ...
    draw_text_top_left(f"Score: {score}", 30, WHITE, surface)
...
def main():
    ...
    draw_text_middle("You Lost", 60, WHITE, screen)
    pygame.display.update()
...
def main_menu():
    ...
    while run:
        screen.fill(BLACK)
        draw_text_middle("Press any key to begin",20, WHITE, screen)
        pygame.display.update()
        for event in pygame.event.get():
            if event.type == pygame.QUIT:
                run = False
